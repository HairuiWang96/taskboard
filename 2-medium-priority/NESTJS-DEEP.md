...
‼️ Nothing above is WRONG. Every line is reasonable. The problem is what
  ...
  6. Every developer solves these same problems in a slightly different way,
...
export class UsersController {
  ...
  create(@Body() dto: CreateUserDto) {  // validation happens automatically
...
class EmailService {
  send(to: string, body: string) { /* calls SendGrid */ }
}
...
@Module({
  imports: [
    ...
    TypeOrmModule.forRoot({ /* connection settings */ }),
