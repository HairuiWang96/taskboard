...
- [Testing — Beginner's Guide](#testing--beginners-guide)
    - [Table of Contents](#table-of-contents)
    - [1. Why Test At All](#1-why-test-at-all)
    - [2. Anatomy of a Test](#2-anatomy-of-a-test)
        - [The assertions you will actually use](#the-assertions-you-will-actually-use)
...
WHAT TESTS COST
  ...
  - False confidence. 100% coverage of the wrong things proves nothing.‼️
...
// you ever write is those three steps. ‼️Keeping them visually separate makes
...
// objects fail. Use toEqual.‼️
expect(obj).toEqual({ id: 1 }); // deep equality — compares contents
expect(obj).toStrictEqual({ id: 1 }); // also checks undefined keys and class‼️
...
expect(user).toMatchObject({ role: 'admin' }); // ‼️ partial match — only‼️
// checks the keys you list

// ── Errors ────────────────────────────────────────────────────────────────
// ‼️ Note the arrow function. Without it the error throws immediately and the
// test crashes instead of passing — the single most common assertion mistake.‼️‼️
...
INTEGRATION TEST
  ...
  ‼️ Best for: the layer where most real bugs live — the seams between parts.
     This is usually the highest-value type of test on a backend.‼️
...
END-TO-END (E2E) TEST
  ...
  ‼️ Best for: a handful of critical user journeys. Not for edge cases.‼️
...
REGRESSION TEST
  ...
  as a failing test first, then fix it. ‼️ Now it cannot silently return.
...
ACCESSIBILITY TEST
  ...
  ‼️ Catches maybe 30% of real issues — the rest needs manual keyboard testing.‼️
...
```text
  ...
  WHY THIS SHAPE:
    ...
    to fail for reasons unrelated to real bugs‼️ (timing, network, a moved
  ...
  ‼️ THE TWO FAILURE MODES, both common:
    ...
    ICE CREAM CONE (inverted)  ‼️Mostly E2E, few unit tests. The suite takes 40
      minutes, fails randomly, and everyone learns to re-run it rather than
      read it. Eventually it is ignored entirely.

    HOURGLASS  ‼️Lots of unit tests and lots of E2E, nothing in between. Every
  ...
  ‼️ THE MODERN ADJUSTMENT — "the Testing Trophy" (Kent C. Dodds):
     ‼️put the MOST weight on INTEGRATION tests, not unit tests. The reasoning:
     ...
     more than ten isolated unit tests of the same code.‼️
```
...
Backend API endpoints (Node)      SUPERTEST‼️                Fires real HTTP
                                                             requests at your
                                                             app without
                                                             binding a port.

Real database in tests            TESTCONTAINERS‼️           Spins up a real
                                                             Postgres/Redis in
                                                             Docker per suite.
...
Faking HTTP calls in tests        MSW (Mock Service Worker)‼️Intercepts at the
                                                             network layer, so
                                                             it works for both
                                                             frontend and
                                                             backend tests.
...
Load testing                      K6 ‼️                      Write load tests
                                                             in JavaScript.

Dependency vulnerabilities        npm audit / SNYK  ‼️       Run in CI.
...
export default defineConfig({
    test: {
        // 'node' for backend-only projects. ‼️'jsdom' simulates a browser
        // environment so React component tests can render into a fake DOM.
        environment: 'jsdom',

        // Makes describe/it/expect available without importing them in every file.‼️
        globals: true,

        // Runs before every test file — where you register jest-dom matchers,
        // start MSW, and so on.‼️
...
describe('UserService.register', () => {
    ...
    it('rejects a duplicate email', async () => {
        ...
        await expect(service.register('ada@example.com', 'a-long-password')).rejects.toThrow(ConflictError);
    ...
    it('rejects a short password', async () => {
        ...
        await expect(service.register('a@b.com', 'short')).rejects.toThrow('Password must be at least 12 characters');
    });
});
```
...
describe('POST /api/users', () => {
    ...
    it('creates a user and returns 201', async () => {
        const response = await request(app).post('/api/users').send({ email: 'ada@example.com', password: 'a-long-password' }).expect('Content-Type', /json/).expect(201);
    ...
    it('returns 409 for a duplicate email', async () => {
        ...
        await request(app).post('/api/users').send({ email: 'dupe@example.com', password: 'a-long-password' }).expect(409);
    });
...
sendEmail.mockRejectedValueOnce(new Error('timeout')).mockResolvedValueOnce({ id: '1' });
...
it('handles the payment provider being down', async () => {
    ...
    server.use(http.get('https://api.stripe.com/v1/customers/:id', () => HttpResponse.json({ error: 'server_error' }, { status: 500 })));
...
jobs:
    test:
        ...
        steps:
            ...
            - run:
                  npm ci # ‼️ `ci` not `install` — installs exactly what
...
await request(app).post('/api/users').set('Authorization', `Bearer ${token}`).send({ email: 'a@b.com' }).expect(201);
