# React + TypeScript — Live Coding Interview Cheat sheet
...
- [React + TypeScript — Live Coding Interview Cheat sheet](#react--typescript--live-coding-interview-cheat-sheet)
    ...
    - [TypeScript-Specific Patterns to Remember](#typescript-specific-patterns-to-remember)
...
```tsx
// ‼️ RefObject<HTMLElement> — the ref can be attached to any HTML element
function useClickOutside(ref: React.RefObject<HTMLElement | null>, callback: () => void): void {
    useEffect(() => {
...
function SearchBox(): JSX.Element {
    ...
    return (
        <div>
            <input value={query} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setQuery(e.target.value)} placeholder='Search...' />
            {loading && <p>Loading...</p>}
            <ul>
                {results.map(item => (
...
function Modal({ isOpen, onClose, children }: ModalProps): JSX.Element | null {
    ...
    return ReactDOM.createPortal(
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onMouseDown={onClose}>
            <div style={{ background: 'white', borderRadius: 8, padding: 24, minWidth: 320 }} onMouseDown={(e: React.MouseEvent) => e.stopPropagation()}>
...
function TodoApp(): JSX.Element {
    ...
    const toggle = (id: number): void => setTodos(prev => prev.map(t => (t.id === id ? { ...t, done: !t.done } : t)));

    const remove = (id: number): void => setTodos(prev => prev.filter(t => t.id !== id));

    return (
        <div>
            <input value={input} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setInput(e.target.value)} onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => e.key === 'Enter' && add()} placeholder='Add todo...' />
...
function Paginated({ data, pageSize = 10 }: PaginatedProps): JSX.Element {
    ...
    const currentItems = useMemo<PaginatedItem[]>(() => data.slice((page - 1) * pageSize, page * pageSize), [data, page, pageSize]);
...
function DragList({ initialItems }: DragListProps): JSX.Element {
    ...
    return (
        <ul style={{ listStyle: 'none', padding: 0 }}>
            {items.map((item, i) => (
                <li key={item.id} draggable onDragStart={() => onDragStart(i)} onDragOver={(e: React.DragEvent) => e.preventDefault()} onDrop={() => onDrop(i)} style={{ padding: '8px 12px', margin: '4px 0', background: '#f0f0f0', cursor: 'grab' }}>
                    {item.name}
                </li>
            ))}
        </ul>
...
function VirtualList({ items, itemHeight = 40, containerHeight = 400 }: VirtualListProps): JSX.Element {
    ...
    return (
        <div style={{ height: containerHeight, overflowY: 'auto', position: 'relative' }} onScroll={(e: React.UIEvent<HTMLDivElement>) => setScrollTop(e.currentTarget.scrollTop)}>
...
type ThemeAction = { type: 'TOGGLE' } | { type: 'SET'; mode: ThemeMode };
    ...
    return (
        <table style={{ borderCollapse: 'collapse', width: '100%' }}>
            <thead>
                <tr>
                    {columns.map(col => (
                        <th key={String(col.key)} onClick={() => handleSort(col.key)} style={{ cursor: 'pointer', padding: '8px 12px', borderBottom: '2px solid #ccc', textAlign: 'left' }}>
                            {col.label}
...
function LazyImage({ src, alt, style, ...props }: LazyImageProps): JSX.Element {
    ...
    return <img ref={imgRef} alt={alt} style={{ opacity: loaded ? 1 : 0, transition: 'opacity 0.3s', background: '#eee', ...style }} {...props} />;
}
```
...
- _"Why `interface` over `type`?"_ → Interfaces are extendable and give better error messages for objects; `type` for unions/intersections
