...
Two microtask queues (processed in order):
  ...
  2. Promise microtask queue (then/catch/finally, queueMicrotask)
...
```js
// setTimeout vs setImmediate outside I/O — ORDER IS NON-DETERMINISTIC
// because timers phase checks if setTimeout has expired; tiny timing differences
...
function recursiveTick() { process.nextTick(recursiveTick) } // ❌ blocks event loop

// setImmediate: runs in check phase — after I/O, won't starve
// Prefer setImmediate for recursive async work

// When to use nextTick:
// - Emit events after constructor finishes (so listener can be attached)
class EventEmitter {
  constructor() {
...
Network I/O (TCP, HTTP) does NOT use thread pool — uses OS epoll/kqueue (async)
...
if (isMainThread) {
  ...
  worker.on('exit', (code) => { if (code !== 0) throw new Error(`Worker stopped: ${code}`) })
} else {
  // Worker thread
...
// Worker pool pattern (Piscina library is the standard):
...
cluster:        multiple Node processes sharing same port (load balancing)
               ...
               Modern alternative: PM2 cluster mode or running multiple containers
```
...
Types:
  ...
  Transform:  Duplex that transforms data (compression, encryption, parsing)
```
...
app.get('/api', (req, res) => {
  ...
  // Fix: use streaming JSON parser, or move to worker thread
})
...
A: Single-threaded event loop + non-blocking I/O via libuv (OS epoll/kqueue).
   Doesn't spin threads per connection (unlike traditional servers).
...
A: Signal from a writable stream that its internal buffer is full.
   ...
   pipe/pipeline manages this automatically.
