...
router.put('/:id', (req, res) => { /* req.user is already loaded */ });
router.delete('/:id', (req, res) => { /* req.user is already loaded */ });
```
...
const asyncHandler = (fn) =>        // fn = your async (req, res) => { ... }
  (req, res, next) =>               // returns Express-compatible middleware
...
const limiter = rateLimit({
  ...
  store: new RedisStore({          // Redis-backed for multi-server consistency‼️
...
const cacheMiddleware = (ttl = 300) => asyncHandler(async (req, res, next) => {
...
app.get('/api/stats', cacheMiddleware(600), asyncHandler(async (req, res) => {
